...
public interface RatesRepository extends CrudRepository<Rates, Integer> {
    ...
    @Query(value = "SELECT * FROM (SELECT * FROM rates WHERE date <= :date ORDER BY date DESC limit :limit) AS subquery ORDER BY date ASC ;", nativeQuery = true, countQuery = "select 1")
